...
// 绘制1位深度图片的函数
void draw1BitImage(uint16_t* fb, int x, int y, int width, int height, const unsigned char* imageData, uint16_t color, int scale = 1) {
  if (scale == 1) {
    // 原始大小绘制
...
void demo05_app() {
  ...
  // 颜色
  const uint16_t COLOR = 0xF81F; // 绿色
  
  while (millis() - startTime < 10000) {
    // 阶段1：显示原始大小的图片（240x240）
    DrawUtil::clear(fb1, 0x0000);
    DrawUtil::clear(fb2, 0x0000);
    
    // 绘制原始大小图片
    draw1BitImage(fb1, 0, 0, IMAGE_WIDTH, IMAGE_HEIGHT, image_1bit, COLOR);
    draw1BitImage(fb2, 0, 0, IMAGE_WIDTH, IMAGE_HEIGHT, image_1bit, COLOR);
...
//    FontRender::drawString(fb2, 70, 10, "原始大小", 0xFFFF);
    ...
    // 绘制缩放后的图片
    draw1BitImage(fb1, scaledX, scaledY, IMAGE_WIDTH, IMAGE_HEIGHT, image_1bit, COLOR, 2);
    draw1BitImage(fb2, scaledX, scaledY, IMAGE_WIDTH, IMAGE_HEIGHT, image_1bit, COLOR, 2);
...
void register_demo05() {
  AppManager::registerApp(0x05, demo05_app);
}
